...
I wrote my own networking code and that part didn't work very well, either. Anyway, it seemed to make sense to attempt to make some version of the game
...
turret to spin a full 360 degrees, lest the wires going to the turret get twisted. I guess that adding a slip-ring was out of the budget. The turret
motor had a clutch system where the gears would slip, instead of stalling the motor. It made a terrible racket, regardless. My very first modification
was to use a Dremel to cut-out the blank portion of the ring gear into something resembling teeth. I added a slip-ring for the wiring. I placed a Pixart
camera on top of the turret and it communicated with an Arduino development board in the main body of the tank. So I had an initial version that could
acquire and track an IR target back and forth. I was also experimenting with ultrasonic distance sensors, as the tank would need some way to navigate
its environment.
...
All the electronics went into the trash. I figured I would only need a few custom parts printed but buying those on-demand was fairly expensive and there
...
So stuck at home during the pandemic, I was left to choose between buying an entire lot of IR-battling robots or purchasing a 3D printer. If I purchased a lot
...
So of course 3D modeling is its own discipline and I never became much more than a hack. I'd consider myself a hack with electronics, also. Software is my actual
profession and the only thing in this project that I feel competent at. 3D printing gears in particular can be a bit tricky, especially as they get smaller. The
...
switch out a bunch of batteries. Because of my reset issues, I reluctantly moved to running the microcontroller off of a 9V battery. But still the reset issues persisted.
...
I did kinda deliberate whether I wanted to pick back up with the LTRT project again. It had last ended in frustration and I spent a lot of time on it without much to show.
 On the other hand, I had spent a lot of time on it without much to show. So I pulled out the electronics. The old, dust-covered Ender 5 came out of the shop. With just
...
more vigorous testing of the robot. Like I said, I'm not really an electronics expert. It's all pretty much magic to me.
...
away from the user. I mean things like keeping track of wheel position and turret position and stuff like that. The code forced some delay between reversing the motors as placing opposite current on a motor while it is still spinning is bad for the motor.
...
bigger. I never really worried about battery life as the motors didn't usually run very long while I was doing development work. Anyway, I had always sorta resisted Lithium
...
In the end, I discovered that 18650 batteries hold a nice charge. A two-pack gives me 7.2 volts to the motors, a nice upgrade from the ~4.5 volts I was seeing with four NiCad batteries.
...
slight redesign in order to eliminate the axles entirely in order accomodate the 18650 pack.
...
use time-of-flight calculations, using infrared light reflecting off of surfaces. That you can buy a module for $5 with circuitry sophisticated enough to get sub-centimeter accuracy
measuring the speed of light is just one of the many wonders of our modern world. Best of all, the VL53L0X boards use I2C to communicate with the microcontroller. Since I was
